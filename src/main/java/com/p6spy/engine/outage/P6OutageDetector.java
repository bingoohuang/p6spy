...
public enum P6OutageDetector implements Runnable {
  ...
  P6OutageDetector() {
    ...
    P6LogQuery.debug("P6Spy - P6OutageOptions.getOutageDetectionIntervalMS() = "
      + P6OutageOptions.getActiveInstance().getOutageDetectionIntervalMS());
  }
  ...
  private void detectOutage() {
    ...
    long ms = P6OutageOptions.getActiveInstance().getOutageDetectionIntervalMS();
    long threshold = TimeUnit.MILLISECONDS.toNanos(ms);
    ...
    for (Object jdbcObject : pendingMessages.keySet()) {
      ...
      if ((currentTime - ii.startTime) > threshold) {
        ...
        P6LogQuery.logElapsed(-1, System.nanoTime() - ii.startTime,
          Category.OUTAGE, ii.preparedStmt, ii.sql, ii.url);
      }
    }
  }
